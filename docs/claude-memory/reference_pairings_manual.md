...
- "Dotación por flota" (manual, reglas transversales section): A320 = 1 TJ + 3 TC (4 cupos), A319 = 1 TJ + 2 TC (3 cupos) — already used by NB. **B767 = 1 TJ + 4 TC (5 cupos); B787 = 6 tripulantes/vuelo (6 cupos)** — confirmed 2026-09-24 from the manual text itself (searched via the docx XML), used for WB's Fase 4 CUADRO FINAL "Cupos" column and for sizing the Fase 1/2 round-robin (`cupos_por_vuelo × 2 legs`, an extension of NB's pattern, not a separately confirmed WB rule).
- `consulta BQ WB.sql` had a typo: `subfleet_code IN ('762', '788', '789')` — B767's real subfleet code is **`763`**, not `762` (per `generar_reporte_pairings.py`'s own comments). Fixed in the WB Colab notebooks (`Automatizacion_WB_Fase3_...`/`Fase4_...`); the original `.sql` file itself was left untouched (legacy reference file, not executed directly).
